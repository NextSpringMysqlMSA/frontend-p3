...
export default function Scope1Form() {
  ...
  return (
    <div className="flex flex-col w-full h-full p-4 pt-24">
      ...
      {/* PageHeader with Factory icon only */}
      <PageHeader
        icon={
          <div className="flex items-center gap-2">
            <button
              onClick={() => router.push('/home')}
              className="flex items-center justify-center w-6 h-6 transition rounded-full hover:bg-gray-100"
              aria-label="뒤로가기">
              <ArrowLeft className="w-4 h-4 text-gray-400 hover:text-blue-600" />
            </button>
      ...
      <LoadingState
        ...
        <div className="space-y-4">{/* 폼 컴포넌트 삽입 위치 */}</div>
      </LoadingState>
    </div>
  )
}
