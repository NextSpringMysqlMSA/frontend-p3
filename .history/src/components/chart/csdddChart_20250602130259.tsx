    ...
    const load = async () => {
      try {
        ...
        setData([
          {
            name: 'EU 공급망 실사',
            isCompleted: eudd.length > 0
          },
          {
            name: '인권 실사',
            isCompleted: hrdd.length > 0
          },
          {
            name: '환경 실사',
            isCompleted: edd.length > 0
          }
        ])
      } catch (err) {
        console.error('자가진단 상태 불러오기 실패:', err)
        // 더미 fallback
        setData([
  ...
  return (
    <div className="space-y-3">
      {data.map(({name, isCompleted}) => (
        <div
          key={name}
          className="flex items-center justify-between px-3 py-2 border rounded border-amber-100 bg-amber-50">
